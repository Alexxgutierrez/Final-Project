...
class HomePage(BasePage):
    ...
    def show_dashboard(self):
        ...
        columns = ("Blood Type", "Donation Amount(mL)")
        dashboard_tree = ttk.Treeview(self.content_frame, columns=columns, show="headings")
        dashboard_tree.heading("Blood Type", text="Blood Type", anchor='center')  # center
        dashboard_tree.heading("Donation Amount(mL)", text="Donation Amount(mL)", anchor='center')  # center

        # Set up center alignment for columns
        for col in columns:
            dashboard_tree.column(col, anchor='center')

        dashboard_tree.pack(padx=10, pady=5)
        
        # Insert data into the Treeview
        for blood_type, donation_amount in blood_type_donations.items():
            dashboard_tree.insert("", "end", values=(blood_type, donation_amount), tags=('center',))  # Center-align values
